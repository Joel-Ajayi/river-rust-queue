...
- **Synchronous (in the merchant's request path):** Merchant → API Gateway → Redis (one XADD) → 202 response. That's it. Anything else happening in the request path would couple merchant latency to internal service health, and we refuse to do that.
...
**Admin CLI.** Operator tooling. Lists DLQ entries, replays them, shows consumer lag per group, shows circuit breaker state per merchant, freezes/unfreezes wallets manually. Not in the request path; runs as a binary the operator invokes. ([→ `15-ADMIN-CLI.md`](services/15-ADMIN-CLI.md))
