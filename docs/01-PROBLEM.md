...
- **Single-Redis-node Redlock is not safe in production.** A real production deployment requires a majority of independent Redis nodes. v1 runs a single node and documents this clearly; the algorithm is unchanged when you scale up.
...
- **DLQs accumulate stale work.** If nobody monitors and drains the DLQ, it grows without bound. RRQ's admin CLI is the operational answer; a healthy production system also alerts on DLQ size.
...
| E. External dependency unavailability | Circuit breaker, exponential backoff with jitter, DLQ, Admin CLI replay |
...
**Not solved (performance):** RRQ targets 1,000 TPS on a single machine. A real payment processor at scale handles tens of thousands. Scaling RRQ to that regime requires sharding the database, partitioning Redis, and running many replicas — well-understood techniques that aren't novel and aren't part of v1.
