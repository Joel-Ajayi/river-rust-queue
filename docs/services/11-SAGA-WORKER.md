...
**Q: Walk me through what happens when the saga worker crashes after Debit but before Credit.**

The Debit step's commit completed before the crash, so `ledger_entries` has the debit row and `saga_state.current_state` is `Debited`. The crashed worker's `XREADGROUP` consumer ID has the message in its pending list, unacked. Sixty seconds later, another worker's `XAUTOCLAIM` reclaims the message. The replacement worker reads `saga_state` for the job_id, sees state is `Debited`, and resumes at the Credit step. The Redlocks have expired by now — the replacement re-acquires them, and re-runs the validation check just to confirm the wallets haven't changed status during the gap. If everything's fine, Credit proceeds and the saga completes. If a wallet got frozen during the gap, the saga transitions to Compensating and undoes the Debit. Either way, the saga reaches a terminal state with the invariants intact.

**Q: Why orchestration over choreography?**

Three reasons. First, our sagas have inter-step data dependencies — the Redlock acquired in step 2 must be released in step 6 by code that knows it's the same logical operation. In choreography, that's awkward; in orchestration, it's natural. Second, the steps need shared error handling logic (retryable vs terminal classification) that's cleaner in one place. Third, the failure path is a totally different shape from the happy path (compensations in reverse), and choreography makes that very hard to read — events flying in opposite directions through different services. With orchestration, the failure path is a method on the orchestrator. The tradeoff is that the Saga Worker becomes a service with significant logic, but that's the right tradeoff for our case.
...
You could, in principle — event sourcing in its pure form derives current state from event history. We don't, for two reasons. First, performance: every saga step would have to replay all of that saga's events to compute current state, which is wasteful when the saga is in flight. Second, recovery semantics: we want to know not just "what happened" but "what should happen next." `saga_state.current_state = 'Debited'` directly tells the resumer "the next step is Credit." With pure event-derived state, the resumer has to walk the event log and infer state, which is more error-prone. The state row is a denormalization that pays back in code simplicity and recovery speed. The events are still the source of truth (reconciliation uses them); the state row is a hot-path cache of "where is the saga now."

**Q: Show me where the type-state pattern actually helps.**

Imagine a refactor where someone adds a new step to the saga, say between Validate and AcquireLock. In Go, they'd update the steps slice, but they'd also need to update every place that reads `saga_state.current_state` and pattern-matches on the string. If they forget one, the bug exists at runtime: a saga is resumed into the wrong state, runs the wrong step, possibly double-debits. The test suite might catch it; might not. In Rust, the new state is a new type, say `PreLocked`. Every function that matched on the old types now needs to handle `PreLocked` — if it doesn't, the match is non-exhaustive and the compiler refuses to build. The bug cannot exist at runtime because the program doesn't compile. That's the qualitative property: bugs that exist in Go's runtime are caught by Rust's type checker.

**Q: What's the worst possible failure for the saga worker?**

A compensation that itself fails permanently. The saga is then in a state where a debit happened but the compensating credit cannot be applied — money is "stuck" in an inconsistent place. The system marks the saga `DeadLettered`, an alert fires, and an operator has to manually decide what to do. The admin CLI lets them inspect the saga's full event history, manually emit an adjustment event with explicit reasoning, and reconcile. This is rare in practice but the system has to make it visible and not paper over it. Silent stuckness is the worst possible failure; loud stuckness with operator tooling is acceptable.

**Q: How do you tune the lock TTL? It seems like a critical knob.**

Empirically. The lock TTL has to be longer than the longest expected duration of the wallet-mutating section of a saga. For a Single Transfer, that's a few hundred milliseconds in the worst case (database round-trips, network jitter). So a TTL of 5 seconds is comfortably above that. For Bulk Payouts, each sub-transfer has its own lock, so the parent payout duration doesn't matter. If you ever see a real saga exceed the TTL — which would show up as another saga acquiring the lock and breaking the first — that's a sign your TTL is too short or your sagas have a hot loop somewhere. There's a watchdog that extends the lock lease for genuinely long sagas; v1 doesn't implement it because the steps are bounded.

**Q: Why not use Postgres advisory locks instead of Redlock?**

Two reasons. First, Postgres advisory locks live for the duration of a database session. The saga spans many database transactions in different sessions (one per step), so the advisory lock doesn't naturally span the saga. We'd have to hold a session open for the whole saga, which doesn't compose well with connection pooling. Second, even if we did that, advisory locks are local to one Postgres instance. If we ever scale to multiple Postgres replicas or shards, advisory locks don't cross those boundaries; Redlock does. Redlock is the more honest answer to "distributed lock across services." That said, Postgres advisory locks are a reasonable simplification for a single-Postgres deployment, and we discuss the simplification in the locking deep-dive.

**Q: What if two workers both XAUTOCLAIM the same stuck message?**

That's possible in principle, though Redis serializes XAUTOCLAIM calls. The defense is the `SELECT ... FOR UPDATE` on the saga_state row when a worker starts processing — only one of the racing workers wins the row lock, the other gets blocked, and by the time it can proceed, the first has likely already advanced the saga's state. The second worker reads the new state, sees the step has already been done, and just ACKs. The Redis-level race is bounded by the database-level row lock.

**Q: How does this design handle the bulk payout backpressure problem?**

The parent BulkPayoutSaga fans out sub-transfers, but it does so through a bounded semaphore. Specifically, when the parent starts, it creates N sub-saga jobs (each with its own job_id and idempotency-key derived from the bulk's key) and writes them to the job stream. The job stream's consumer count and Redis's natural pacing become the backpressure. The parent doesn't try to hold all 5000 sub-transfers in memory simultaneously; it batches the writes and tracks completions via a counter in saga_state. The bulk-payout's terminal state ("BulkPayoutCompleted") is reached when the counter equals N.

**Q: What does the runtime cost of all this state tracking look like?**

Each saga step does roughly: one row read (saga_state, for-update), one row insert (ledger_entry), one row insert (event), one row update (saga_state). At 1,000 TPS, that's 4,000 writes/sec to Postgres, comfortably within modern SSD capacity. The state-row pattern makes the database the bottleneck before anything else; we monitor it specifically. In benchmarks, the bottleneck has historically been Postgres write throughput, not application code.
